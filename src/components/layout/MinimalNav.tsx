...
export const MinimalNav = () => {
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 bg-zinc-darkest/80 backdrop-blur-sm border-b border-zinc-800/50 transition-all duration-300">
      <div className="container mx-auto px-4 md:px-6 py-2 md:py-2.5 flex items-center justify-between relative">
        ...
        >
          <Link 
            to={ROUTES.HOME} 
            className="font-logo text-sm md:text-base lg:text-lg uppercase tracking-[0.02em] block leading-none text-white transition-all duration-200 min-h-[44px] flex items-center"
          >
            IMANOL VILLAGOMEZ
          </Link>
        </motion.div>
            ...
            <AnimatePresence>
              {isWorkDropdownOpen && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  transition={{ duration: 0.2 }}
                  className="absolute top-full left-0 mt-2 bg-zinc-darkest/95 backdrop-blur-sm border border-zinc-800/50 min-w-[120px]"
                  onMouseEnter={() => setIsWorkDropdownOpen(true)}
                  onMouseLeave={() => setIsWorkDropdownOpen(false)}
                >
                  <Link
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0 }}
              transition={{ duration: 0.2 }}
              className="md:hidden fixed inset-0 bg-zinc-darkest/50 z-30"
        ...
        <AnimatePresence>
          {isMobileMenuOpen && (
            <motion.div
              ...
              exit={{ opacity: 0, y: -20 }}
              transition={{ duration: 0.2 }}
              className="md:hidden fixed top-full left-0 right-0 bg-zinc-darkest/95 backdrop-blur-sm border-b border-zinc-800/50 z-40 shadow-lg"
